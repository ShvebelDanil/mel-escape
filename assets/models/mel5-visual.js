...
export function createMelVisual(THREE, GFX, options = {}) {
  ...
  // Футболка — внутренний слой. Глубина уменьшена (.34 вместо .38): при .38
  // её передняя грань ложилась ровно в z=.19, как передняя грань куртки, а
  // задняя — в z=-.23, как задняя грань куртки. Две совпадающие плоскости
  // давали z-fighting (мигание куска туловища). Теперь футболка полностью
  // внутри куртки.
  put(inner, rounded(.62, .66, .34, tee, .025), 0, 1.24, -.02);
  ...
  // Задняя часть воротника. Высота и центр те же, что у лацканов (.22 при y=1.60,
  // т.е. 1.49..1.71) — воротник идёт ровной лентой вокруг шеи, а не задирается
  // вверх капюшоном. Глубина .30 (z -.34..-.04) уводит его за затылок (z≈-.30),
  // а по x (±.27) и z он заходит внутрь лацканов (x .18..30, z -.17..27) —
  // три части сливаются в один замкнутый воротник без щелей.
  put(inner, rounded(.54, .22, .30, leatherMat, .03), 0, 1.60, -.19);
  ...
  function leg(x) {
    ...
    // Шаг и количество подобраны так, чтобы последняя планка (z=.22)
    // оставалась внутри подошвы (она кончается на z=.335): раньше шестая
    // планка попадала в z=.36 и висела в воздухе перед носком.
    for (let i = 0; i < 5; i++) {
      put(l, box(.28, .012, .015, '#0a0a0a'), 0, bootY - .092, -.14 + i * .09);
    }

    put(l, box(.25, .05, .12, '#050505'), 0, bootY - .085, -.11);

    // Шнуровка по передней панели голенища (z≈.143), с подъёмом вверх.
    // Раньше ряды уезжали вперёд на носок (z до .20) на высоте, где
    // поверхности ботинка уже нет, — перекладины висели в воздухе.
    for (let i = 0; i < 5; i++) {
      const y = bootY + .030 + i * .021;
      put(l, box(.12, .012, .018, lace), 0, y, .143);
      put(l, sphere(.011, metal), -.06, y, .143);
      put(l, sphere(.011, metal), .06, y, .143);
    }
