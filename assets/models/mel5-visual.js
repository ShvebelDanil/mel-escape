...
//   // --- Procedural textures ---
//   function leatherTex(repeatX, repeatY) {
...
//       g.fillStyle = Math.random() > 0.5 ? `rgba(60,60,64,${a})` : `rgba(10,10,12,${a})`;
...
//         sx + (Math.random() - 0.5) * 200, sy + (Math.random() - 0.5) * 200,
//         sx + (Math.random() - 0.5) * 260, sy + (Math.random() - 0.5) * 260
//       );
//       g.stroke();
//     }
...
//       g.fillStyle = `rgba(80,80,84,${0.04 + Math.random() * 0.05})`;
//       g.fill();
//     }
...
//       g.beginPath();
//       g.moveTo(0, yy);
//       g.lineTo(512, yy + (Math.random() - 0.5) * 30);
//       g.setLineDash([4, 4]);
//       g.stroke();
//     }
//     g.setLineDash([]);
...
//   function denimTex(repeatX, repeatY) {
//     const c = document.createElement('canvas');
...
//       g.beginPath();
//       g.arc(x, y, r, 0, Math.PI * 2);
//       g.fillStyle = `rgba(90,100,120,${0.05 + Math.random() * 0.06})`;
//       g.fill();
//     }
...
//   const leatherMat = leatherTex(1.6, 1.4);
//   const denimMat = denimTex(2.2, 2.2);
...
//   // Main tee torso, broader and with a belly.
//   put(inner, rounded(.62, .66, .38, tee, .025), 0, 1.24, 0);

//   // Jacket back and sides.
//   put(inner, rounded(.76, .70, .42, leatherMat, .03), 0, 1.26, -.02);
//   put(inner, rounded(.22, .68, .43, leatherMat, .025), -.29, 1.26, .02);
//   put(inner, rounded(.22, .68, .43, leatherMat, .025), .29, 1.26, .02);

//   // Belly overlay — tinted to match the black tee exactly (previously used
//   // the lighter "leather" tone, which stood out as a mismatched grey patch
//   // against the pure black shirt) and scaled down ~12% so it reads as a
//   // modest gut under the shirt instead of dominating the torso silhouette.
...
//   // --- Mohawk ---
//   // Built as a continuous chain of overlapping clumps that follow the real
//   // curvature of the skull (so there are no bald gaps underneath), each
//   // topped with a handful of chaotically-angled spikes for a wild, ragged
//   // edge. The sweep direction gradually tilts forward at the front and back
//   // at the rear, like a real mohawk, instead of pure random noise — that is
//   // what keeps a dense/chaotic look from reading as "crooked".
...
//     const baseY = skullTopY(z) - .020;     // embed slightly so it reads as rooted hair, not floating

//     const clump = new THREE.Group();
//     clump.position.set(0, baseY, z);

//     // Forward sweep at the front, backward sweep at the back.
//     const sweep = (t - .5) * .85;
...
//     const spikeCount = 3 + Math.round(Math.random() * 2);
//     for (let s = 0; s < spikeCount; s++) {
//       const h = (.11 + volume * .21) * (0.72 + Math.random() * .55);
//       const rBase = .020 + Math.random() * .014;
//       const spike = new THREE.Mesh(new THREE.ConeGeometry(rBase, h, 5), mat(hair));
//       const jitterX = (Math.random() - 0.5) * (.018 + volume * .048);
//       const jitterZ = (Math.random() - 0.5) * .034;
//       spike.position.set(jitterX, h * .48, jitterZ);
//       spike.rotation.x = sweep + (Math.random() - 0.5) * .30;
//       spike.rotation.z = (Math.random() - 0.5) * .48;
//       spike.rotation.y = (Math.random() - 0.5) * .38;
//       clump.add(spike);
//     }
...
//   // --- Eyes ---
//   // Clean, tidy construction: soft socket shading, a smooth almond-shaped
//   // white, a crisp flat-disc iris/pupil, a slim eyelid arc and a properly
//   // placed brow — instead of a stack of loosely overlapping spheres.
//   //
//   // NOTE on `ez`: the skull is an ellipsoid (scale .29/.33/.30, center
//   // y=.02). At this eye's (x,y) the *actual* skull surface sits at
//   // z≈0.283 — so the whole assembly must sit at/just past that depth or it
//   // gets swallowed by the head and becomes invisible (this was the bug).
//   // ez=.273 puts the eyeball/lid/brow ~0.005-0.02 proud of the skull
//   // surface — enough to read clearly without looking like a bolted-on
//   // googly eye. All the *relative* offsets below (iris/pupil/catchlight/
//   // lids/brow) are unchanged from the original, tidy design.
//   for (const side of [-1, 1]) {
//     const ex = side * .092, ey = .048, ez = .273;

//     // Soft recessed socket shading (subtle, not a hard dark ring).
//     const socket = put(headG, sphere(1, skinShadow, 14, 10), ex, ey - .004, ez - .014);
//     socket.scale.set(.082, .050, .022);

//     // Eye white — smooth almond shape, sitting just under the brow.
//     const eye = put(headG, sphere(1, '#f5f0e6', 16, 14), ex, ey, ez);
//     eye.scale.set(.066, .038, .024);

//     // Iris — crisp flat disc, reads as a clean circle rather than a bulge.
//     put(headG, discZ(.024, '#4d5d52'), ex, ey, ez + .022);
//     // Pupil.
//     put(headG, discZ(.012, '#0a0b0a'), ex, ey, ez + .024);
//     // Catchlight.
//     put(headG, discZ(.005, '#ffffff'), ex - .010, ey + .010, ez + .025);

//     // Upper eyelid — slim arc that hugs the eye for definition.
//     const upperLid = put(headG, rounded(.076, .015, .028, skinShadow, .006), ex, ey + .026, ez - .002);
//     upperLid.rotation.z = side * .05;

//     // Fine crease line above the lid.
//     put(headG, rounded(.070, .005, .022, '#6b4f3d', .002), ex, ey + .036, ez + .006);

//     // Lower lash line — subtle, skin-toned.
//     put(headG, rounded(.064, .008, .020, skin, .004), ex, ey - .024, ez - .004);

//     // Eyebrow — slightly angled for a rugged look, positioned properly above the eye.
//     const brow = put(headG, rounded(.096, .020, .022, '#4a3b30', .006), ex, ey + .068, ez - .006);
//     brow.rotation.z = side * -.08;
//   }

//   // --- Nose ---
//   // Simplified into three clean forms instead of five overlapping shadow-colored blobs.
...
//   put(headG, rounded(.010, .030, .008, skinShadow, .004), 0, -.082, .256);

//   // --- Mouth ---
//   // The original had no mouth geometry at all, which is what left the lower
//   // face reading as an unshaped lumpy mess — added a proper closed-lip mouth.
//   const mouthY = -.122;
...
//     // Thick leather sleeve (integrated shoulder).
//     put(a, rounded(.285, .62, .275, leatherMat, .028), 0, -.24, 0);
//     // Extra shoulder bulk at top of sleeve.
...
// All animation pivots and returned references match the original buildMel().
//
// This is an optimization pass over the original createMelVisual(): the
// external result (geometry, positions, rotations, scales, colors, pivot
// structure) is byte-for-byte identical to before. Only internal plumbing
// was tightened: duplicated texture-generation code was merged into one
// shared factory, and two tiny helpers (rnd/jitter) replace the many
// inline `a + Math.random()*(b-a)` / `(Math.random()-0.5)*s` expressions
// that were scattered across the file. No visible shape, color or
// proportion was changed.
export function createMelVisual(THREE, GFX, options = {}) {
  const root = new THREE.Group();
  const put = (p, o, x = 0, y = 0, z = 0) => { o.position.set(x, y, z); p.add(o); return o; };
