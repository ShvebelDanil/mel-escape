...
export function createMelVisual(THREE, GFX, options = {}) {
  ...
  // ---- backpack (fixed straps) ----
  // The straps are now built as two pieces per side that stay OUTSIDE the
  // torso's solid volume at all times: a yoke that caps the shoulder
  // (sitting above the torso's rounded top, bridging to the pack on the
  // back) and a diagonal chest strap that runs just in front of the shirt
  // (z beyond the shirt's front face) down to a small buckle near the belt.
  for (const side of [-1, 1]) {
