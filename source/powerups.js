...
// Магнит: радиус сбора и физика полёта чекушки к игроку.
const MAG_RANGE = 14;        // на сколько метров вперёд дотягивается, все три ряда
const MAG_V0 = 6;            // стартовая скорость подхвата, м/с
const MAG_ACC = 95;          // ускорение, м/с² — к концу полёта чекушка идёт быстрее игрока
...
// На такой высоте игрок перелетает ряды чекушек и пикапы выше окна сбора (обычно ±1.2..1.5 м)
// и они остаются позади. Пока бафф активен, окно и у чекушек (main.js), и у пикапов (ниже в этом
...
// MAX WIN. Смещение пары чекушек внутри ряда по X: ряды разнесены на 2.3 м, полуширина
...
// и бафф включался бы и выключался с такой же задержкой. Вместо этого чекушки раздваиваются
// на подлёте игрока: у каждой в окне DBL_RANGE заводится близнец, и пара разъезжается из одной
// точки за ~0.25 с — подмена читается как «чекушка раздвоилась», а не как хлопок из ниоткуда.
...
// ─── визуал пикапа ───────────────────────────────────────────────────────────────
// Квад с иконкой + мягкий нимб за ним, оба развёрнуты по камере (как чекушки). Это 2 draw
...
export function update(dt, px, py, pz) {
  ...
  let got = null;
  const pcy = py + 0.95;
  // В сапогах Мэл перелетает пикап выше обычного окна сбора — та же логика, что у чекушек
  ...
  for (let i = activePickups.length - 1; i >= 0; i--) {
    ...
    // Окно сбора по Z — тот же порядок, что у чекушек (0.95): на просадке до 20 fps кадр
...
// Магнит. Чекушки не телепортируются: каждой заводится собственный вектор скорости, который
// доворачивается на игрока — получается дуга, а не рывок по прямой. Полей у описателя чекушки
...
// Летящие под магнитом чекушки пропускаем целиком: их ведёт собственная скорость, и переписывать
...
export function updateDouble(dt, pz) {
  ...
  for (let i = coins.length - 1; i >= 0; i--) {
    ...
    if (on && !c.pair && c.z > pz && c.z < far) {
      // Фаза покачивания у пары общая — иначе в момент разъезда чекушки идут «вразнобой».
