...
export function buildClassroom() {
  ...
  GFX.put(g, GFX.box(5.0, 2.6, 0.12, '#5d4634'), 0, 2.7, U.CLASS_Z1 + 0.06); GFX.put(g, GFX.box(4.7, 2.3, 0.03, '#1f3029'), 0, 2.7, U.CLASS_Z1 + 0.135); GFX.put(g, GFX.box(5.0, 0.08, 0.18, '#5d4634'), 0, 1.38, U.CLASS_Z1 + 0.2);
...
// Цвета полотна доски-стойки: «чёрная» и два оттенка тёмно-зелёного — три равновероятных
// варианта одного препятствия, выбираются при сборке экземпляра пула. Полотно держим тёмным нарочно:
// картинки серии boardN рисуют белым мелом по прозрачному фону, и им нужен контраст.
const BOARD_COLORS = ['#2b2e33', '#1f3029', '#26392e'];
...
function buildObstacle(type) {
  ...
  // под полотном пусто: стойки и лыжи вынесены за |x| > hw (0.86), перекладин внизу нет.
    ...
    GFX.put(g, GFX.box(1.98, 1.01, 0.03, '#1f3029'), 0, 1.735, -0.062);                         // подложка под текстуру (края доски)
