...
- **Object pooling (обязателен по правилам проекта)**: `obstaclePool{type:[]}`/`activeObstacles[]` + `spawnObstacle/releaseObstacle/clearObstacles`; `activeCoins[]` + `spawnCoin/releaseCoin`. Переиспользуются не только меши, но и **описатели** (`obDescPool`/`coinDescPool`) — раньше `spawnObstacle` создавал новый объект через `Object.assign`. Группам препятствий выставлен `matrixAutoUpdate = false`, матрица считается один раз в `spawnObstacle`. Ни один спавн не создаёт новый `THREE.Mesh`, если пул непуст.
- **Тени препятствий**: `initObstacleShadows()` — один `InstancedMesh` (круг ×64) на все препятствия вместо отдельного прозрачного диска у каждого (было до 20 draw call за кадр, стало 1). Матрицы пишутся только в `spawnObstacle`/`releaseObstacle` — препятствия не двигаются относительно мира, поэтому в кадре работы ноль. Освобождение — swap-remove: последний инстанс переезжает в освободившийся слот, владелец правится через `shadowOwner[]`.
