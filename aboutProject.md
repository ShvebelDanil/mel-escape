...
- `player` — позиция/физика игрока: `lane` (0..2), `x/y/z`, `vy`, `grounded`, `rolling`, `invuln`, `runPhase`, плюс привод вращения корпуса `spin`/`spinDir`/`spinT` (см. «Вращение корпуса» ниже).
...
**Вращение корпуса** (`updateSpin()` в `animatePlayer`): перекат и сальто крутят один и тот же `node.pivot.rotation.x` через общий привод `player.spin` (накопленный угол), `player.spinDir` (`+1` вперёд, `-1` назад, `0` — вращения нет) и `player.spinT` (длительность оборота). Поворот по `+X` наклоняет голову по ходу движения, поэтому перекат всегда `+1` (кувырок **вперёд**). Скорость идёт по дуге (медленнее на входе/выходе, быстрее в группировке), группировка `k = sin(πp)` поджимает конечности, голову и корпус «в клубок» — за счёт этого голова не проходит сквозь пол. Если приземлились раньше, чем докрутили сальто (например, на парту), остаток доворачивается за 0.16с.
